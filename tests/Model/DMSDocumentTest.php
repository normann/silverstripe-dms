...
use SilverStripe\DMS\Tests\Stub\StubRelatedDocumentExtension;
...
class DMSDocumentTest extends SapphireTest
{
    ...
    public function testDefaultDownloadBehabiourCMSFields()
    {
        ...
        Config::inst()->update(DMSDocument::class, 'default_download_behaviour', 'open');
        $cmsFields = $document->getCMSFields();
        $this->assertEquals('open', $cmsFields->dataFieldByName('DownloadBehavior')->Value());


        Config::inst()->update(DMSDocument::class, 'default_download_behaviour', 'download');
        $cmsFields = $document->getCMSFields();
    ...
    {
        DMSDocument::add_extension(StubRelatedDocumentExtension::class);
    ...
    {
        Config::inst()->update(DMS::class, 'folder_name', 'assets/_unit-tests');
